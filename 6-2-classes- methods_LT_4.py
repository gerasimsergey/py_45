# в этой части кода провел рефакторинг (для методов сравнения) и реализовал сравнение
# так же написал код для "полевого" испытания
...
class Lecturer(Mentor):
    ...
    # Надо бы вынести в класс выше или наследовать всех от Person или как то так
    def get_grade_for_curs(self, curs):
        result = 0;
        curs_grades = self.grades.get(curs)
        if not curs_grades:
